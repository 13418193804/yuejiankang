...
function doPay(req, res, opt) {
  ...
  let moneytest = 40000
  if (req.cookies.openid == "opoj20qqrV7JH-l8o6vC9Ty4guSQ") {
    moneytest = 1
  }
  ...
  //   },
    ...
    //     function (err, response, body) {
    //     });
    //   if (req.cookies.openid == "opoj20kMSZogWTFg7gSE0VQIyx8E" ||req.cookies.openid ==  "opoj20gd5aSykeaGXJbp1DPjZjA4") {
    //     res.send({data: body,
    //       sec_order_id: opt.outTradeNo,
    //       appId: config.wechat.appid});
    //   }
    // });
    ...
    request.post({
      ...
      function (err, response, body) {
        if (!err && response.statusCode == 200) {
          ...
          if (bodydata.code == 1) {
            doPay(req, res, {detail: '低剂量螺旋CT肿瘤标记物⾎液检查(复查).', outTradeNo: outTradeNo, spbillIp: spbillIp});
          } else {
            res.send(body);
          }
        }

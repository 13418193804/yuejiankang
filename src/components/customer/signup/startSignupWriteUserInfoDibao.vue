<template>
  <div class="content-wrapper">
    ...
    <div class="weui-cell input-wrapper">
      <div class="weui-cell__hd">
         <label class="weui-label" style='font-size:0.8rem'><span style="color:#F43530">*&nbsp;</span>低保证号/低收入证号/五保证号</label>
      </div>
      <div class="weui-cell__bd">
          <input class="weui-input" placeholder="请输入正确的证号" v-model="userInfo.safeid" @blur="checkSafeId()">
      </div>
     </div>
    </div>
    <div class="weui-cell input-wrapper">
      <div class="weui-cell__hd">
         <label class="weui-label"><span style="color:#F43530">*&nbsp;</span>有效期至</label>
      </div>
      <div class="weui-cell__bd">
          <input class="weui-input" placeholder="格式为2017-01" v-model="userInfo.safedate" @blur="checkSafeExpiryData()">
      </div>
    </div>
    <div class="content-second">
       <div>
        ...
        <p style="font-size:0.9rem;color:rgb(145,145,145)">
          3.检测时请带上<span style="color:#71a0d1">⾝份证,低保证明</span>进⾏签到</p>
       </div>
    </div>
    <div class="btn-area">
...
export default {
  ...
  computed: {
    uploadstatus() {
      if (this.inUploading === 1) return "开始报名";
      else if (this.inUploading === 2) return "上传数据";
    }
  },
  created() {
    this.$AppCacheKey.setPeopleType(4);
  },
  methods: {
    uploadUserSignUpInfo() {
      ...
      if (!this.userInfo.code || !/(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[0-9Xx]$)/.test(this.userInfo.code)) {
        this.options.content = "身份证格式不对";
        this.showNotification = true;
        return;
      }
      if (!this.userInfo.safeid || this.userInfo.name.safeid < 8) {
        this.options.content = "请输入正确的低保号";
        this.showNotification = true;
        return;
      } 
    
      if (!this.userInfo.safedate || !/^(\d{4})-(0\d{1}|1[0-2])$/.test(this.userInfo.safedate)) {
        this.options.content = "请输入正确的有效时间";
        this.showNotification = true;
        return;
      } 
      this.inUploading = 2;
      ...
      let userdata =
        ...
        this.userInfo.sex +
        "&dibao_code=" +
        this.userInfo.safeid +
        "&dibao_date=" +
        this.userInfo.safedate +
        "&open_id=" +

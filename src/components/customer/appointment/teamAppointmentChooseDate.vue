      ...
      <div v-show="isShowContent" class="first-item-wrapper weui-form-preview__bd" v-for="(appointmentDate,index) in appointmentDates" 
          @click="appointmentDate.user_num > 0 && onItemClick(appointmentDate,index)" :key="index" :class="{'first-item-select':currentIndex === index}">
           <div class="weui-form-preview__item">
             ...
             <span class="weui-form-preview__value">{{appointmentDate.user_num>0?'可预约':'已满员'}}</span>
           </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    chooseDate() {
      ...
      let that = this;
      let url = webconfig.webserverurl + "/api/order/teamAreas";
      let params = {
      ...
      this.$wrapperHttp.get(this, params, url, function(data) {
        if (data) {
          ...
          for (let i = 0; i < data.length; i++) {
            let item = data[i];
            if(item.team_use > 0){
              item.user_num = item.team_use;
              tempData.push(item);
            }
          }
          that.appointmentDates = tempData;
